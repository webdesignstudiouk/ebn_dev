...
class CED extends Controller
{
    ...
    public function report_new($prospect_type = 1, $report = false) {
        ...
        } elseif ($prospect_type == 2) {
            $prospects = $model->where('type_id', $prospect_type, ($report ? '100' : ''))->where('deleted_at', null);
        ...
        foreach ($prospects as $prospect) {
            ...
            if ($prospect_type == 1) {
                $ced_label = 'From Prospect Verbal CED';
                $skip = ($prospect->verbalCED ? false : true);
                $ced_date = \Carbon\Carbon::createFromFormat('d/m/Y', $prospect->verbalCED);
            } elseif ($prospect_type == 2) {
                ...
                if (is_iterable($prospect->sites)) {
                    foreach ($prospect->sites as $site) {
                        if ($site->lowestGasMeter) {
                            ...
                            if(!$lowest_ced || $ced_date < $lowest_ced){
                                $lowest_ced = $ced_date;
                            }
                        }

                        if ($site->lowest_electric_meter) {
                            $ced_date = \Carbon\Carbon::parse($site->lowest_electric_meter->contractEndDate);
                            $electric_ceds[] = [
                                'site_id' => $site->id,
                                'meter' => $ced_date
                            ];
                ...
                $skip = ($lowest_ced ? false : true);

                if(!$skip) {
                    // Debug
//                    dd($electric_ceds, $gas_ceds);
                    $ced_date = \Carbon\Carbon::parse($lowest_ced);
                }
            }

            // Skip if cant find ced
            if (!$skip) {

                $ced_diff_months = $ced_date->diffInMonths(\Carbon\Carbon::now());
                $ced_diff_days = $ced_date->diffInDays(\Carbon\Carbon::now());
                ...
                $user_data = [
                    ...
                    'agent' => $agent_name,
                    'verbal_ced_is_past' => $ced_date->isPast(),
                    'verbal_ced_is_today' => $ced_date->isToday(),
                    'verbal_ced' => $ced_date->format('d/m/Y'),
                    'verbal_ced_diff_in_days' => $ced_diff_days,
