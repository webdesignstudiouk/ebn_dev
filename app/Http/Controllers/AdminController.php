...
//            ->with('cards', $cards);
    ...
    {
        if ($type_id == "deleted") {
            $prospects = Prospects::withTrashed()->with('user')->where('deleted_at', '!=', null)->where('user_id', '!=', 2)->where('user_id', '!=', 100)->orderBy('company', 'asc');
        } else {
            $prospects = Prospects::with('user')->where('type_id', $type_id)->where('user_id', '!=', 2)->where('user_id', '!=', 2)->where('user_id', '!=', 100)->orderBy('company', 'asc');
        }
