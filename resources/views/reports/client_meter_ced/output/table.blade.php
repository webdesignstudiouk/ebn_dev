        ...
        @foreach($counts['traffic_lights'] as $count_key => $count)
            <div class="col-sm-3">
                <div class="xe-widget xe-counter" style="text-align: center; border-bottom: 2px solid {{($count_key == 'danger' ? '#cc3f44' : ($count_key == 'warning' ? '#ffba00' : '#a6ce39'))}};">
                    <div class="xe-label">
                        <strong class="num">{{count($counts['traffic_lights'][$count_key]['meters'])}}<small style="font-size: 12px"> {{( count($data) != 0 ? number_format((count($counts['traffic_lights'][$count_key]['meters']) / count($data)) * 100, 2): 0)}}%</small></strong>
                ...
                @foreach($data as $d)
                    <tr>
                        ...
                        @if($d['ced_is_past'])
                            <td></td>
                        @else
                            <td><span class="badge">{{ $d['ced_diff_in_days'] }} Until End Date</span></td>
                        @endif
                        <td><span class="badge badge-{{$d['ced_traffic_light']}}">o</span></td>
