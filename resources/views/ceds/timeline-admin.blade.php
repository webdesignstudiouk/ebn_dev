...
<ul class='cbp_tmtimeline'>
	@php
	$distinctCED = $prospectModal->select(DB::raw("STR_TO_DATE( verbalCED ,'%d/%m/%Y' ) as date"))->distinct()->where('verbalCED', '!=', '')->orderBy('date')->pluck('date');
	@endphp
	...
	$count = $prospectModal->where('verbalCED', Carbon\Carbon::createFromFormat('Y-m-d', $c1)->format('d/m/Y'))->count();
	@endphp
	<li>
		...
		<div class='collapse' id='{{$collapseId}}'>
			@foreach($prospectModal->where('verbalCED', Carbon\Carbon::createFromFormat('Y-m-d', $c1)->format('d/m/Y'))->get() as $c2)
					...
					<div class='xe-current-day'>
						<div class='xe-now'>
							<div class='xe-temperature' style='color:#000!important;'>
								<h3>{{$c2->company}}</h3>								<p>{{$c2->user->first_name}} {{$c2->user->second_name}}</p>  								
							</div>
						</div>
					</div>
